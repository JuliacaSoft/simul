...
class EnsayoControlador {
    ...
    function formEditEnsayo() {
        ...
        if ($Tipo == 1) {
            ...
            $depend = $this->model->buscarAreasConocimiento();
            $ensayo = $this->model->buscarIdEnsayoArea($Ensayoid);
        } elseif ($Tipo == 3) {
            $ensayo = $this->model->buscarIdEnsayoGrupo($Ensayoid);
            $depend = $this->model->buscarGrupoProcesos();
        }
